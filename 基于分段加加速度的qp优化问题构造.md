...
$$
\ddot s_{i+1} = \ddot s_i + \dddot s_i * dt_i \label{scalar_acc_trans}
$$

$$
\dot s_{i+1} = \dot s_i +\ddot s_i dt_i + \dfrac{1}{2}\dddot s_i dt^2_i \label{scalar_vel_trans}
$$

$$
s_{i+1} = s_i + \dot s_i dt_i + \dfrac{1}{2}\ddot s_i dt^2_i + \dfrac{1}{6}\dddot s_i dt^3_i \label{scalar_dis_trans}
$$

为了更方便地代入到二次优化问题中，公式$\eqref{scalar_acc_trans}-\eqref{scalar_dis_trans}$需要转换成矩阵形式如下
$$
\ddot S = \ddot S_0 + A*T*\dddot S \label{mat_acc_trans}
$$

$$
\dot S = \dot S_0 + AT\ddot S_0 + (ATBT+\dfrac{1}{2}ATT)\dddot S \label{mat_vel_trans}
$$

$$
S = S_0 + AT\dot S_0 + (ATBT+\dfrac{1}{2}ATT)\ddot S_0 + (ATBTBT+\dfrac{1}{2}ATBTT + \dfrac{1}{2}ATTBT + \dfrac{1}{6}ATTT)\dddot S \label{mat_dis_trans}
$$

其中，
$$
\dddot S = [\dddot s_0, \dddot s_1,...,\dddot s_{K-2},\dddot s_{K-1}]^T \label{eq_mat_sddd}
$$

$$
\ddot S = [\ddot s_1, \ddot s_2,...,\ddot s_{K-1},\ddot s_{K}]^T
$$

$$
\dot S = [\dot s_1, \dot s_2,...,\dot s_{K-1},\dot s_{K}]^T
$$

$$
S = [s_1, s_2,...,s_{K-1},s_{K}]^T
$$

$$
\ddot S_0 = [\ddot s_0,...,\ddot s_0]^T \in R^K
$$

$$
\dot S_0 = [\dot s_0, ...,\dot s_0]^T \in R^K
$$

$$
S_0 = [s_0, ...,s_0]^T \in R^K
$$
...
\vdots & \vdots & \ddots & 0 \\
1 & 1 & \dots & 1
\end{bmatrix}_{K \times K} \label{eq_mat_A}
$$
...
1 & \dots & 1 & 0
\end{bmatrix}_{K \times K} \label{eq_mat_B}
$$
...
\vdots & \ddots & \ddots & 0 \\
0 & \dots & 0 & dt_{K-1}
\end{bmatrix}_{K\times K} \label{eq_mat_T}
$$
...
$$
f(\dddot S) = f_1(S, S_{ref}) + f_2(\dot S, \dot S_{ref}) + f_3(\ddot S, \ddot S_{ref}) + f_4(\dddot S, \dddot S_{ref}) \label{total_cost}
$$
其中，
$$
f_1(S, S_{ref}) = (S-S_{ref})^TW_1(S-S_{ref}) \label{dis_cost_simple}
$$

$$
f_2(\dot S, \dot S_{ref}) = (\dot S-\dot S_{ref})^TW_2(\dot S- \dot S_{ref}) \label{vel_cost_simple}
$$

$$
f_3(\ddot S, \ddot S_{ref}) = (\ddot S-\ddot S_{ref})^TW_3(\ddot S-\ddot S_{ref}) \label{acc_cost_simple}
$$

$$
f_4(\dddot S, \dddot S_{ref}) = (\dddot S-\dddot S_{ref})^TW_4(\dddot S-\dddot S_{ref}) \label{jerk_cost_simple}
$$
...
$$
f_1(S, S_{ref}) = \dddot S^TC^TW_1C\dddot S + 2D^TW_1C\dddot S + D^TW_1D \label{dis_cost}
$$
函数$f_2(\dot S, \dot S_{ref})$展开合并如下
$$
f_2(\dot S, \dot S_{ref}) = \dddot S^T E^T W_2 E \dddot S + 2F^T W_2 E \dddot S + F^T W_2 F \label{vel_cost}
$$
函数$f_3(\ddot S, \ddot S_{ref})$展开合并如下
$$
f_3(\ddot S, \ddot S_{ref}) = \dddot S^T M^T W_3 M \dddot S + 2 N^T W_3 M \dddot S + N^T W_3 N \label{acc_cost}
$$
函数$f_4(\dddot S, \dddot S_{ref})$展开合并如下
$$
f_4(\dddot S, \dddot S_{ref}) = \dddot S^T P^T W_4 P \dddot S + 2 Q^T W_4 P \dddot S + Q^T W_4 Q \label{jerk_cost}
$$
...
$$
S_{low} \leq S \leq S_{upp} \label{dis_ineq}
$$

$$
\dot S_{low} \leq \dot S \leq \dot S_{upp} \label{vel_ineq}
$$

$$
\ddot S_{low} \leq \ddot S \leq \ddot  S_{upp} \label{acc_ineq}
$$

$$
\dddot S_{low} \leq \dddot  S \leq \dddot  S_{upp} \label{jerk_ineq}
$$
...
$$
\begin{align}
& minimize \space f(\dddot S) = \dfrac{1}{2}\dddot S^T H \dddot S + U^T \dddot S \label{qp_cost} \\
& subject \space to \space \space g(\dddot S) = G \dddot S \leq L \label{qp_constraints}
\end{align}
$$
具体推导如下
...
\begin{align}
f(\dddot S) = 
& \dddot S^T C^T W_1 C \dddot S + 2D^T W_1 C \dddot S D^T W_1 D + \\
...
$$
H = 2(C^T W_1 C + E^T W_2 E + M^T W_3 M + P^T W_4 P)
$$

$$
U = 2(D^T W_1 C + F^T W_2 E + N^T W_3 M + Q^T W_4 P)^T
$$
...
\vdots \\
\dddot s_{K-1}
\end{bmatrix}_{K \times 1} \label{eq_acc_complicated}
$$
...
\vdots \\
\dddot s_{K-1}
\end{bmatrix}_{K \times 1} \label{eq_explain1}
$$
...
\vdots \\
\dddot s_{K-1}
\end{bmatrix}_{K \times 1} \label{eq_explain2}
$$
...
\vdots \\
\dddot s_{K-1}
\end{bmatrix}_{K \times 1} \label{eq_explain3}
$$
对上式合并同类项，可得
$$
\begin{bmatrix}
...
\vdots \\
\dddot s_{K-1}
\end{bmatrix}_{K \times 1} \label{eq_explain4}
$$
...
> 向量$[\dot s_0, \dot s_1, ..., \dot s_{K-1}]^T$的第2\~K-1行算式用公式$\eqref{eq_acc_explain6}$的第1\~K-1表示，向量$[\dot s_0, \dot s_1, ..., \dot s_{K-1}]^T$的第1行用$\dot s_0 + 0$表示。
...
\vdots \\
\dddot s_{K-1}
\end{bmatrix}_{K \times 1} \label{eq_explain8}
$$
对上式合并同类项，可得
$$
\begin{bmatrix}
